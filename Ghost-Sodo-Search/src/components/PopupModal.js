...
function SearchBox() {
    ...
    if (!searchValue) {
        className = 'z-10 relative flex items-center py-5 px-4 sm:px-7 bg-white rounded-lg dark:bg-slate-800';
    }
    ...
    return (
        <div className={className} ref={containerRef}>
            ...
            <input
                ...
                className='grow -my-5 py-5 -ml-3 pl-3 text-[1.65rem] focus-visible:outline-none placeholder:text-gray-400 outline-none truncate dark:bg-slate-800 dark:text-white'
...
function SearchClearIcon() {
    ...
    if (!searchValue) {
        return (
            <SearchIcon className='text-neutral-900 dark:text-white' alt='Search' />
        );
    }
    return (
        <button alt='Clear' className='-mb-[1px]' onClick={() => {
            ...
            <ClearIcon className='text-neutral-900 hover:text-neutral-500 h-[1.1rem] w-[1.1rem] dark:text-white hover:dark:text-gray-400' />
        </button>
    );
}
...
function CancelButton() {
    ...
    return (
        <button
            className='ml-3 text-sm text-neutral-500 sm:hidden' alt='取消'
            onClick={() => {
                dispatch('update', {
                    showPopup: false
                });
...
function TagListItem({tag, selectedResult, setSelectedResult}) {
    ...
    return (
        ...
        >
            ...
            <h2 className='text-[1.65rem] font-medium leading-tight text-neutral-900 truncate dark:text-white'>{name}</h2>
        </div>
    );
}
...
function PostListItem({post, selectedResult, setSelectedResult}) {
    ...
    return (
        ...
        >
            <h2 className='text-[1.65rem] font-medium leading-tight text-neutral-800 dark:text-gray-400'>
...
function HighlightWord({word, isExcerpt}) {
    ...
    return (
        <>
            <span className='font-bold text-neutral-900 dark:text-white'>{word}</span>
        </>
    );
}
...
function ShowMoreButton({posts, maxPosts, setMaxPosts}) {
    ...
    return (
        <button
            className='w-full my-3 p-[1rem] border border-neutral-200 hover:border-neutral-300 text-neutral-800 hover:text-black font-semibold rounded transition duration-150 ease hover:ease dark:text-gray-400 hover:dark:text-white'
            onClick={() => {
...
function Search() {
    ...
    return (
        <>
            ...
            >
                <div className='bg-white w-full max-w-[95vw] sm:max-w-lg rounded-lg shadow-xl m-auto relative translate-z-0 animate-popup dark:bg-slate-800'>
                    <SearchBox />
                    <SearchResultBox />
                </div>
            </div>
